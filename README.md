...
## [🚀 AuditPilot 사이트 바로 열기](https://auditpilot.streamlit.app/)
...
## 설계 방향: 규칙 기반 업무 처리와 LLM 지식 보완

AuditPilot의 핵심 감사업무 처리는 사전에 정의한 규칙과 계산 로직을 기반으로
구현했습니다. 자료 판별, 헤더 매핑, 중복·결측 확인, 차변·대변 검증, 명세서
대사, 증감분석과 승인 게이트처럼 결과의 일관성과 재현성이 중요한 기능은
Python 코드에 명시된 기준에 따라 동작합니다.

이 구조를 선택한 목적은 민감한 감사자료를 불필요하게 외부 AI 서비스에
전송하지 않고, 어떤 조건으로 결과가 만들어졌는지 확인할 수 있도록 하기
위함입니다. 업로드한 원장과 명세서 전체가 OpenAI API로 자동 전송되지는
않습니다. 다만 규칙 기반 구현만으로 모든 회계·감사 질문과 예외 상황을
설명하기에는 한계가 있으므로, 부족한 지식과 사용자 안내는 LLM API가 연결된
`삼일이 AI` 챗봇으로 보완합니다.

역할은 다음과 같이 분리되어 있습니다.

- **규칙 기반 영역:** 자료 정리, 데이터 검증, 계산, 이상치 후보 산출과 승인
  절차
- **LLM 보완 영역:** 사용자가 챗봇에 직접 입력한 질문에 대한 설명, 감사업무
  흐름 안내와 참고 답변
- **감사인 판단 영역:** 위험평가, 예외 해석, 추가 절차 결정과 최종 결론 승인

LLM 응답은 참고용이며 감사 결론으로 자동 확정되지 않습니다. 챗봇 질문은
OpenAI API 처리를 위해 외부로 전송될 수 있으므로 사용자는 질문창에 실제
API 키, 개인정보 또는 원장 원문과 같은 민감정보를 입력하지 않아야 합니다.
API 키는 브라우저나 GitHub에 포함하지 않고 서버 실행 환경에서만 불러옵니다.
